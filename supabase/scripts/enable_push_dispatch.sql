...
SET
  edge_url = 'https://ffkxsqflgbmdqzkmacwb.supabase.co/functions/v1/send-push',
  hook_secret = 'oprjasegpoijwresapogfu3w890fg734u2fjqopjdfio&&fd3qp',
  enabled = true,
  updated_at = now()
WHERE id = 1;
...
-- Diagnostics (look at results):
SELECT * FROM public.push_dispatch_config WHERE id = 1;

SELECT COUNT(*) AS tokens FROM public.user_push_tokens;

SELECT id, title, error, processed_at, created_at, data->>'kind' AS kind
FROM public.push_jobs
